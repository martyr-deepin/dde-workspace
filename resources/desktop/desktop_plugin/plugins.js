...
(function() {
  var DEEPIN_ITEM_ID, Loader, Weather, ajax, allname, assert, build_menu, calc_text_size, clamp, create_element, create_img, dnd_is_deepin_item, dnd_is_desktop, dnd_is_file, echo, find_drag_target, get_function_name, get_page_xy, plugins, prov2city, run_post, swap_element, weather, _,
  ...
  prov2city = {
    ...
    '湖北': ['襄樊', '荆门', '黄冈', '恩施', '武汉', '麻城', '黄石', '鄂州', '孝感', '咸宁', '随州', '仙桃', '天门', '潜江', '神农架', '枣阳'],
  ...
  Weather = (function() {
    function Weather() {
      ...
      var city_and_date, close, closebutton, img_more_url_init, img_now_url_init, more_city, week_init,
        _this = this;
      ...
      this.element = create_element("div", "Weather", null);
      this.element.draggable = true;
      this.weathergui_init();
      this.weather_now_pic = create_img("weather_now_pic", img_now_url_init, this.element);
      this.temperature_now = create_element("div", "temperature_now", this.element);
      this.temperature_now.textContent = "20°";
      more_city = create_element("div", "more_city", this.element);
      this.city = create_element("div", "city", more_city);
      this.city.textContent = "请选择城市";
      ...
      this.more_city_img = create_img("more_city_img", this.img_url_first + "ar.png", more_city);
      ...
      close = create_element("div", "close", this.element);
      closebutton = create_img("closebutton", this.img_url_first + "closebutton.png", close);
      ...
      this.date.addEventListener("click", function() {
        if (_this.more_weather_menu.style.display === "none") {
          ...
          _this.more_city_menu.style.display = "none";
          return _this.more_weather_menu.style.zIndex = "1";
        }
      });
      more_city.addEventListener("click", function() {
        var key, provincetemp, _results;

        if (_this.more_city_menu.style.display === "none") {
          _this.more_city_menu.style.display = "block";
          _this.chooseprov.style.display = "block";
          _this.choosecity.style.display = "block";
          _this.more_weather_menu.style.display = "none";
          _this.more_city_menu.style.zIndex = "65535";
        } else {
          _this.more_city_menu.style.display = "none";
          _this.chooseprov.style.display = "none";
          _this.choosecity.style.display = "none";
          _this.more_weather_menu.style.display = "none";
          _this.more_city_menu.style.zIndex = "1";
        }
        _this.chooseprov.size = 13;
        _this.chooseprov.options.length = 0;
        _results = [];
        for (key in prov2city) {
          provincetemp = create_element("option", "provincetemp", _this.chooseprov);
          _results.push(provincetemp.innerText = key);
        }
        return _results;
      });
      this.chooseprov.addEventListener("change", function() {
        var citytemp, cityvalue, provIndex, _i, _len, _ref, _results;

        echo("prov change");
        provIndex = _this.chooseprov.selectedIndex;
        _this.provincevalue = _this.chooseprov.options[provIndex].value;
        echo(_this.provincevalue);
        if (prov2city[_this.provincevalue].length < 13) {
          _this.choosecity.size = prov2city[_this.provincevalue].length;
        } else {
          _this.choosecity.size = 13;
        }
        _this.choosecity.options.length = 1;
        _ref = prov2city[_this.provincevalue];
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          cityvalue = _ref[_i];
          citytemp = create_element("option", "citytemp", _this.choosecity);
          _results.push(citytemp.innerText = cityvalue);
        }
        return _results;
      });
      this.chooseprov.addEventListener("blur", function() {
        return echo("prov blur");
      });
      this.choosecity.addEventListener("blur", function() {
        return echo("city blur");
      });
      this.chooseprov.addEventListener("focus", function() {
        return echo("prov focus");
      });
      this.choosecity.addEventListener("focus", function() {
        return echo("city focus");
      });
      this.choosecity.addEventListener("change", function() {
        var auto_update, ci, cityIndex, cityid, provin, _results;

        echo("city change");
        _this.chooseprov.style.display = "none";
        _this.choosecity.style.display = "none";
        cityIndex = _this.choosecity.selectedIndex;
        _this.cityvalue = _this.choosecity.options[cityIndex].value;
        echo(_this.cityvalue);
        _results = [];
        for (provin in allname.data) {
          if (allname.data[provin].省 === _this.provincevalue) {
            _results.push((function() {
              var _results1;

              _results1 = [];
              for (ci in allname.data[provin].市) {
                if (allname.data[provin].市[ci].市名 === this.cityvalue) {
                  echo(allname.data[provin].市[ci].编码);
                  cityid = allname.data[provin].市[ci].编码;
                  this.cityurl = "http://m.weather.com.cn/data/" + cityid + ".html";
                  _results1.push(auto_update = setInterval(this.weathergui_update(this.cityurl), 10 * 60000));
                } else {
      ...
      this.refresh.addEventListener("click", function() {
        ...
        return _this.weathergui_update(_this.cityurl);
      });
    }
    ...
    Weather.prototype.weathergui_init = function() {
      return window.loader.addcss('desktop_plugin/weather/weather.css', 'screen print').load();
    };

    Weather.prototype.weathergui_update = function(url) {
      var _this = this;

      return this.ajax(url, "GET", function(xhr) {
        var i, i_week, j, str_data, temp_str, temper, weather_data, week_n, week_name;
        ...
        week_n = i_week;
        _this.weather_now_pic.src = _this.img_url_first + "48/T" + weather_data.weatherinfo.img1 + weather_data.weatherinfo.img_title1 + ".png";
        ...
        _this.temperature_now.textContent = Math.round(temper) + "°";
        _this.city.textContent = weather_data.weatherinfo.city;
        _this.week1.textContent = week_name[week_n % 7];
        _this.pic1.src = _this.img_url_first + "24/T" + weather_data.weatherinfo.img1 + weather_data.weatherinfo.img_title1 + ".png";
        _this.temperature1.textContent = weather_data.weatherinfo.temp1;
        _this.week2.textContent = week_name[(week_n + 1) % 7];
        _this.pic2.src = _this.img_url_first + "24/T" + weather_data.weatherinfo.img3 + weather_data.weatherinfo.img_title3 + ".png";
        _this.temperature2.textContent = weather_data.weatherinfo.temp2;
        _this.week3.textContent = week_name[(week_n + 2) % 7];
        _this.pic3.src = _this.img_url_first + "24/T" + weather_data.weatherinfo.img5 + weather_data.weatherinfo.img_title5 + ".png";
        _this.temperature3.textContent = weather_data.weatherinfo.temp3;
        _this.week4.textContent = week_name[(week_n + 3) % 7];
        _this.pic4.src = _this.img_url_first + "24/T" + weather_data.weatherinfo.img7 + weather_data.weatherinfo.img_title7 + ".png";
        _this.temperature4.textContent = weather_data.weatherinfo.temp4;
        _this.week5.textContent = week_name[(week_n + 4) % 7];
        _this.pic5.src = _this.img_url_first + "24/T" + weather_data.weatherinfo.img9 + weather_data.weatherinfo.img_title9 + ".png";
        _this.temperature5.textContent = weather_data.weatherinfo.temp5;
        _this.week6.textContent = week_name[(week_n + 5) % 7];
        _this.pic6.src = _this.img_url_first + "24/T" + weather_data.weatherinfo.img11 + weather_data.weatherinfo.img_title11 + ".png";
