...
(function() {
  ...
  find_free_position = function(w, h) {
    ...
    for (i = _j = 0; 0 <= cols ? _j <= cols : _j >= cols; i = 0 <= cols ? ++_j : --_j) {
      for (j = _k = 0; 0 <= rows ? _k <= rows : _k >= rows; j = 0 <= rows ? ++_k : --_k) {
        if (!(o_table[i][j] != null)) {
          info.x = i;
          info.y = j;
          return info;
      ...
      _results.push((function() {
        ...
        for (j = _k = 0; 0 <= rows ? _k <= rows : _k >= rows; j = 0 <= rows ? ++_k : --_k) {
          if (o_table[i][j] != null) {
            ...
            _results1.push(ctx.clearRect(i * i_width, j * i_height, i_width - 5, i_height - 5));
          }
        }
        return _results1;
      })());
  ...
  sort_item = function() {
    var item, x, y, _j, _len, _ref, _results;
    _ref = $(".item");
    _results = [];
  ...
  Item = (function(_super) {
    ...
    function Item(name, icon, exec, path) {
      ...
      if (typeof this.init_drag === "function") {
        this.init_drag();
      }
      if (typeof this.init_drop === "function") {
        this.init_drop();
      }
    }
