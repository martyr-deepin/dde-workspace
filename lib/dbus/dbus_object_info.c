...
struct DBusObjectInfo* build_object_info( GDBusConnection* con, const char *server, const char* path, const char *interface)
{
    ...
    GError* error = NULL;
    GDBusProxy* proxy = g_dbus_proxy_new_sync(con, G_DBUS_PROXY_FLAGS_NONE, NULL, server, path, "org.freedesktop.DBus.Introspectable", NULL, &error);
    if (error != NULL) {
    ...
    char* r = g_variant_get_child_value(args, 0);
    char* info_xml = g_variant_get_string(r, NULL);
