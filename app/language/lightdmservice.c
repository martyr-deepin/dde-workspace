...
#define LIGHTDM_DBUS_NAME     "com.deepin.dde.lightdm"
#define LIGHTDM_DBUS_OBJ       "/com/deepin/dde/lightdm"
#define LIGHTDM_DBUS_IFACE     "com.deepin.dde.lightdm"
...
const char* _lightdm_dbus_iface_xml =
"<?xml version=\"1.0\"?>\n"
"<node>\n"
"	<interface name=\""LIGHTDM_DBUS_IFACE"\">\n"
"		<method name=\"UnlightdmCheck\">\n"
...
static guint lightdm_service_owner_id;
static guint lightdm_service_reg_id;        //used for unregister an object path
static guint retry_reg_timeout_id;   //timer used for retrying dbus name registration.
static GDBusConnection* lightdm_connection;
...
static void _bus_handle_exit_lightdm (const gchar *username, const gchar *password);

static gboolean _bus_handle_need_pwd (const gchar *username);

static gboolean _bus_handle_is_livecd (const gchar *username);

static gboolean _bus_handle_unlightdm_check (const gchar *username, const gchar *password);
...
void
lightdm_setup_dbus_service ()
{
    GError* error = NULL;

    node_info = g_dbus_node_info_new_for_xml (_lightdm_dbus_iface_xml, &error);
    if (error != NULL) {
        g_critical ("Unable to parse interface xml: %s", error->message);
        g_error_free (error);
    }

    interface_info = g_dbus_node_info_lookup_interface (node_info, LIGHTDM_DBUS_IFACE);
    if (interface_info == NULL) {
        g_critical ("Unable to find interface '"LIGHTDM_DBUS_IFACE"'");
    }

    lightdm_service_owner_id = 0;
    lightdm_service_reg_id = 0;
...
{
    lightdm_service_owner_id = g_bus_own_name (G_BUS_TYPE_SYSTEM,
                                            LIGHTDM_DBUS_NAME,
                                            G_BUS_NAME_OWNER_FLAGS_NONE,
                                            lightdm_service_reg_id ? NULL : _on_bus_acquired,
                                            _on_name_acquired,
                                            _on_name_lost,
                                            NULL,
                                            NULL);
...
_on_bus_acquired (GDBusConnection * connection, const gchar * name G_GNUC_UNUSED, gpointer user_data)
{
    ...
    lightdm_connection = connection;

    //register object.
    lightdm_service_reg_id = g_dbus_connection_register_object (connection,
                                                             LIGHTDM_DBUS_OBJ,
                                                             interface_info,
                                                             &interface_table,
                                                             user_data,
    ...
    if (error != NULL) {
        ...
        g_bus_unown_name (lightdm_service_owner_id);
...
_on_name_lost (GDBusConnection * connection, const gchar * name G_GNUC_UNUSED, gpointer user_data G_GNUC_UNUSED)
{
    if (connection == NULL) {
        ...
        g_critical("Unable to claim the name %s", LIGHTDM_DBUS_NAME);
    }
                  ...
                  gpointer user_data G_GNUC_UNUSED)
{
    ...
    if (g_strcmp0 (method, "Exitlightdm") == 0) {

        const gchar *username = NULL;
        const gchar *password = NULL;
        g_variant_get (params, "(ss)", &username, &password);

        _bus_handle_exit_lightdm (username, password);

    } else if (g_strcmp0 (method, "UnlightdmCheck") == 0) {

        const gchar *username = NULL;
        const gchar *password = NULL;
        g_variant_get (params, "(ss)", &username, &password);

        retval = g_variant_new("(b)", _bus_handle_unlightdm_check(username, password));
        ...
        g_warning ("Calling method '%s' on lightdm and it's unknown", method);
    }

    if (error != NULL) {

        g_dbus_method_invocation_return_dbus_error (invocation, "com.deepin.dde.lightdm.Error", error->message);
        g_error_free (error);
...
static void
_bus_handle_exit_lightdm (const gchar *username, const gchar *password)
{
    gchar *lightdmpid_file = g_strdup_printf ("%s%s%s", "/home/", username, "/.dlightdmpid");

    if (!g_file_test (lightdmpid_file, G_FILE_TEST_EXISTS)) {
        g_debug("user hadn't lightdmed");

    } else {
        gchar *contents = NULL;
        gsize length;

        if (g_file_get_contents (lightdmpid_file, &contents, &length, NULL)) {

            gboolean succeed = _bus_handle_unlightdm_check (username, password);
            g_debug ("kill user lightdm by pid");
            ...
            if (succeed) {
                ...
                if (kill ((pid_t)strtol (contents, NULL, 10), SIGTERM) == 0){
                    g_debug ("kill user lightdm succeed");

                } else {
                    g_debug ("kill user lightdm failed");
                }
            ...
            g_warning("get lightdmpid contents failed");
        }

        g_free(contents);
    }

    g_free(lightdmpid_file);
}

static gboolean
_bus_handle_unlightdm_check (const gchar *username, const gchar *password)
{
